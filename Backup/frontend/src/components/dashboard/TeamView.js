        ...
        <table className="w-full text-left text-sm text-neutral-700">
          ...
          <tbody className="divide-y divide-neutral-100">
            {users.map((targetUser, idx) => (
              <tr key={targetUser.id} className="hover:bg-neutral-50 transition-colors">
                  ...
                  {canManage(user.role, targetUser.role) && (
                    <div className="flex gap-2 justify-end items-center">
                      ...
                      {(user.role === 'admin' || user.role === 'superadmin') && (
                        targetUser.status === 'active'
                          ? <button onClick={() => onBlockUser(targetUser.id, true)} className="p-1 hover:bg-amber-50 text-neutral-400 hover:text-amber-600 rounded transition-colors" title="Block User"><Ban className="w-4 h-4" /></button>
                          : <button onClick={() => onBlockUser(targetUser.id, false)} className="p-1 hover:bg-emerald-50 text-neutral-400 hover:text-emerald-600 rounded transition-colors" title="Unblock User"><Unlock className="w-4 h-4" /></button>
                      )}
                    </div>
                  )}
                </td>
