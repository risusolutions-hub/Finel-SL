...
export default function EngineerTicketsView({
  ...
  const myTickets = complaints.filter(c => 
    parseInt(c.assignedTo) === parseInt(currentUser?.id) && 
    ['assigned', 'in_progress'].includes(c.status)
  );
  
  // Available: pending tickets that have no assignee
  const availableTickets = complaints.filter(c => 
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex gap-3">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-100 rounded-md">
            ...
            <span className="text-sm font-bold text-emerald-900">{availableTickets.length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {[
          ...
          { id: 'available', label: 'Available', count: availableTickets.length },
      ...
      {displayedTickets.length === 0 ? (
        ...
        <div className="space-y-4">
          {displayedTickets.map(ticket => {
            ...
            const machine = getMachine(ticket.machineId);
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap justify-end">
                      ...
                      {ticket.status === 'pending' && !ticket.assignedTo && (
                        <button
                          onClick={() => onTakeTicket && onTakeTicket(ticket.id)}
                      ...
                      {ticket.status === 'in_progress' && isMyTicket && (
                        <button
                          onClick={() => onCancelAssignment && onCancelAssignment(ticket.id)} // Using cancel callback for 'Pause' as in old code
