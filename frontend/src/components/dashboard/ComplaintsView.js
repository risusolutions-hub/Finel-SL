            ...
            <tbody className="divide-y divide-slate-100">
              {filteredComplaints.length > 0 ? (
                filteredComplaints.map((c, idx) => {
                  ...
                  const cKey = c.id || c._id || c.complaintId;
                  const finalKey = cKey ? String(cKey) : `generated-${idx}`;
                  const selectedKey = selectedComplaint ? (selectedComplaint.id || selectedComplaint._id || selectedComplaint.complaintId) : null;
                  const isSelected = selectedKey && selectedKey === finalKey;
                  return (
                    <tr key={finalKey} className={`hover:bg-slate-50 transition-colors ${isSelected ? 'bg-blue-50' : ''}`}>
