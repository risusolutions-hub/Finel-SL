...
import { 
  ...
  AlertCircle, Play, CheckCircle, XCircle, Eye, ChevronDown, ChevronUp,
  Building, Package, Hash, MessageSquare, ArrowRight, Ban, Pause
} from 'lucide-react';
import StatusBadge from './StatusBadge';

export default function EngineerTicketsView({
  complaints,
  customers,
  machines,
  currentUser,
  ...
  // Wait for currentUser to be loaded before filtering
  if (!currentUser) {
    return (
  ...
  // Filter complaints for engineer view
  // My Tickets: assigned to me AND status is assigned or in_progress (active work)
  const myTickets = complaints.filter(c => {
    const assigned = c.assignedTo && (safeId(c.assignedTo._id) || safeId(c.assignedTo.id) || safeId(c.assignedTo));
    const current = safeId(currentUser?._id) || safeId(currentUser?.id);
    const isMine = assigned && current && assigned === current && ['assigned', 'in_progress'].includes(c.status);
    try {
      if (assigned || current) {
        console.debug('[MYTICKETS] Ticket', c.id, 'assignedTo:', assigned, 'currentUser:', current, 'status:', c.status, 'isMine:', isMine);
      }
    } catch (e) {}
    return isMine;
  });

  // Available: pending tickets that have no assignee
      ...
      (ticket, index, self) =>
        ...
        self.findIndex(
          (t) =>
            t.id === ticket.id ||
            t._id === ticket._id ||
            t.complaintId === ticket.complaintId
        )
    );

  const uniqueAvailableTickets = getUniqueTickets(
    complaints.filter(c => {
      const assigned = c.assignedTo && (safeId(c.assignedTo?._id) || safeId(c.assignedTo?.id) || safeId(c.assignedTo));
      return c.status === 'pending' && (!assigned || assigned === '' || assigned === 'null' || assigned === 'undefined');
    })
  );

  const availableTickets = uniqueAvailableTickets; // Ensure compatibility with existing references
  
  // History: resolved or closed tickets
  const historyTickets = complaints.filter(c => 
    ['resolved', 'closed'].includes(c.status)
  );
  
  const displayedTickets = filter === 'my' 
    ? myTickets 
    : filter === 'available' 
      ? uniqueAvailableTickets 
      : historyTickets;

  const getCustomer = (customerId) => customers.find(c => c.id === customerId);
  const getMachine = (machineId) => machines.find(m => m.id === machineId);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
        ...
        <div className="flex gap-3">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-100 rounded-md">
            ...
            <span className="text-sm font-bold text-blue-900">{myTickets.length}</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 border border-emerald-100 rounded-md">
            ...
            <span className="text-sm font-bold text-emerald-900">{uniqueAvailableTickets.length}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {[
          ...
          { id: 'available', label: 'Available', count: uniqueAvailableTickets.length },
          { id: 'history', label: 'Closed', count: historyTickets.length }
        ].map(tab => (
          <button
            key={tab.id}
      ...
      {displayedTickets.length === 0 ? (
        ...
        <div className="space-y-4">
          {displayedTickets.map((ticket, idx) => {
            const customer = getCustomer(safeId(ticket.customerId));
            const machine = getMachine(safeId(ticket.machineId));
            const keyId = safeId(ticket.id) || safeId(ticket._id) || safeId(ticket.complaintId);
            const finalKey = keyId || `ticket-${idx}`;
            // Debug: log computed keys to help trace duplicate/undefined key warnings
            try { console.debug('EngineerTicketsView key:', { idx, id: safeId(ticket.id), _id: safeId(ticket._id), complaintId: safeId(ticket.complaintId), finalKey }); } catch(e){}
            const ticketApiId = safeId(ticket._id) || safeId(ticket.id) || safeId(ticket.complaintId) || `ticket-${idx}`; // prefer Mongo _id when calling APIs
            const isExpanded = expandedTicket === finalKey;
            const assignedForTicket = ticket.assignedTo && (safeId(ticket.assignedTo._id) || safeId(ticket.assignedTo.id) || safeId(ticket.assignedTo));
            const isMyTicket = assignedForTicket && String(assignedForTicket) === String(safeId(currentUser?._id) || safeId(currentUser?.id));

            return (
              <div 
                key={finalKey}
                className={`bg-white rounded-lg border border-slate-200 shadow-sm hover:border-slate-300 border-l-4 ${getStatusBorderColor(ticket.status)} transition-all`}
              >
                {/* Main Card Content */}
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    ...
                    <div className="flex-1 space-y-3">
                      ...
                      <div className="flex items-start gap-4">
                        <div className="mt-1">
                          <span className="text-xs font-mono font-bold text-slate-500 block">#{ticket.displayId || ticket.complaintId || (safeId(ticket.id) ? `#${safeId(ticket.id)}` : `#${idx}`)}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap justify-end">
                      ...
                      {ticket.status === 'pending' && !ticket.assignedTo && (
                        <button
                          onClick={() => onTakeTicket && onTakeTicket(ticketApiId, currentUser?._id || currentUser?.id)}
                          className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors text-xs font-bold uppercase tracking-wide"
                        >
                      ...
                      {ticket.status === 'assigned' && isMyTicket && (
                        <button
                          onClick={() => onStartWork && onStartWork(ticketApiId, 'in_progress')}
                          className="flex items-center gap-1.5 px-3 py-2 bg-amber-500 text-white rounded-md hover:bg-amber-600 transition-colors text-xs font-bold uppercase tracking-wide"
                        >
                      ...
                      {ticket.status === 'assigned' && isMyTicket && (
                        <button
                          onClick={() => onCancelAssignment && onCancelAssignment(ticketApiId)}
                          className="flex items-center gap-1.5 px-3 py-2 border border-red-200 text-red-700 hover:bg-red-50 rounded-md transition-colors text-xs font-bold uppercase tracking-wide"
                        >
                      ...
                      {ticket.status === 'in_progress' && isMyTicket && (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700 transition-colors text-xs font-bold uppercase tracking-wide"
                        >
                      ...
                      {ticket.status === 'in_progress' && isMyTicket && (
                        <button
                          onClick={() => onCancelAssignment && onCancelAssignment(ticketApiId)}
                          className="flex items-center gap-1.5 px-3 py-2 border border-amber-200 text-amber-700 hover:bg-amber-50 rounded-md transition-colors text-xs font-bold uppercase tracking-wide"
                        >
