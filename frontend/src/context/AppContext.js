...
export function AppProvider({ children, user, onUserUpdate }) {
  ...
  const loadData = useCallback(async () => {
      console.log('[AppContext] loadData called. user:', user);
    try {
      setLoading(true);
      ...
      if (user?.role === 'engineer') {
        ...
        let myRes, availableRes;
        try {
          myRes = await api.get('/complaints/my');
          console.log('[API] /complaints/my response:', myRes.data);
        } catch (err) {
          console.error('[API ERROR] /complaints/my', err);
          myRes = { data: { complaints: [] } };
        }
        try {
          availableRes = await api.get('/complaints/available');
          console.log('[API] /complaints/available response:', availableRes.data);
        } catch (err) {
          console.error('[API ERROR] /complaints/available', err);
          availableRes = { data: { complaints: [] } };
        }
        myComplaints = myRes.data.complaints || [];
        availableComplaints = availableRes.data.complaints || [];
        complaints = [...myComplaints, ...availableComplaints];
        try {
  ...
  const assignComplaint = useCallback(async (complaintId, engineerId) => {
    ...
    try {
      await api.post(`/complaints/${complaintId}/assign`, { engineerId, ticketId: complaintId });
      loadData();
  ...
  const cancelAssignment = useCallback(async (complaintId) => {
    try {
      await api.post(`/complaints/${complaintId}/unassign`, { ticketId: complaintId });
      loadData();
  ...
  const updateTicketStatus = useCallback(async (id, status) => {
    try {
      await api.put(`/complaints/${id}/status`, { status, ticketId: id });
      loadData();
  ...
  const closeTicket = useCallback(async (complaintId, notes) => {
    ...
    try {
      await api.post(`/complaints/${complaintId}/close`, { solutionNotes: notes, ticketId: complaintId });
      loadData();
